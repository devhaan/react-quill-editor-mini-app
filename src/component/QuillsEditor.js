import React, { useState, useEffect } from "react";
import ReactQuill from "react-quill"; // ES6
import "react-quill/dist/quill.snow.css"; // ES6

function QuillsEditor(props) {
  const [text, setText] = useState("");
  const [edit, setEdit] = useState(false);
  const [pageText, setPageText] = useState([]);
  const [editold, setEditOld] = useState(false);
  const [textold, setTextold] = useState();
  const [indexes, setIndex] = useState(-1);
  const [twoOp, settwoOp] = useState(false);

  const [imagecheck, setImageCheck] = useState(false);
  const [image, setImage] = useState([]);
  const [imageText, setImageText] = useState("");

  useEffect(() => {}, [text, pageText, image]);

  const onClickAddText = () => {
    setPageText([...pageText, text]);
    setEdit(false);
    settwoOp(false);
  };
  const onClickAddTextold = () => {
    if (indexes !== -1) {
      pageText.splice(indexes, 1, textold);
    }
    setEditOld(false);
    settwoOp(false);
  };

  const onClickeditOld = (item, index) => {
    console.log("dev", item);
    setTextold(item);
    setIndex(index);
    setEditOld(true);
    settwoOp(false);
  };
  const handleClickImage = () => {
    setImage([...image, imageText]);
  };

  return (
    <div>
      {pageText.map((item, index) => (
        <>
          <div className="d-flex flex-column m-3 align-items-center">
            <div>{item}</div>
            <div className="w-10 m-2">
              <button
                className="btn btn-outline-primary"
                onClick={() => onClickeditOld(item, index)}
              >
                Edit
              </button>
            </div>
          </div>
        </>
      ))}

      {image.map((item) => (
        <>
          <img src={item} alt="/" />
        </>
      ))}

      {edit && (
        <>
          <ReactQuill value={text} onChange={(value) => setText(value)} />

          <button onClick={onClickAddText}>ADD</button>
        </>
      )}
      {editold && (
        <>
          <ReactQuill value={textold} onChange={(value) => setTextold(value)} />

          <button onClick={onClickAddTextold}>ADD</button>
        </>
      )}

      {imagecheck && (
        <>
          Enter Imagae Link
          <input onChange={(e) => setImageText(e.target.value)} type="text" />
          <button onClick={handleClickImage}>Add Image</button>
        </>
      )}
      {twoOp && (
        <>
          <div style={{ width: "35vh" }} className="d-flex flex-column w-20">
            <b>Choose</b>
            <button onClick={() => setEdit(true)}>
              <div className="d-flex flex-row">
                <img
                  style={{ width: "6vh" }}
                  alt="/"
                  src="data:image/jpeg;base64,"
                />
                <div className="d-flex flex-column">
                  <b>Text</b>
                  <span>Just start wrriting plain text</span>
                </div>
              </div>
            </button>

            <button onClick={() => setImageCheck(true)}>
              <div className="d-flex flex-row">
                <img
                  style={{ width: "6vh" }}
                  alt="/"
                  src="data:image/jpeg;base64,"
                />
                <div className="d-flex flex-column">
                  <b>Text</b>
                  <span>Upload image or link</span>
                </div>
              </div>
            </button>
          </div>
        </>
      )}
      <button
        className="btn btn-outline-primary"
        onClick={() => settwoOp(!twoOp)}
      >
        +
      </button>
    </div>
  );
}

export default QuillsEditor;
